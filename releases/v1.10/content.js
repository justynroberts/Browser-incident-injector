...
(function() {
    ...
    // Trusted Types policy — some sites (incl. ServiceNow, Google) enforce
    // `require-trusted-types-for 'script'`, which rejects raw string
    // assignments to innerHTML/outerHTML and string args to document.write.
    // We create a pass-through policy; all HTML we assign is extension-owned,
    // so there's no injection risk from user-controlled input here.
    const trustedHTMLPolicy = (() => {
        try {
            if (window.trustedTypes && typeof window.trustedTypes.createPolicy === 'function') {
                return window.trustedTypes.createPolicy('incident-injector', {
                    createHTML: (s) => s,
                    createScriptURL: (s) => s,
                    createScript: (s) => s
                });
            ...
            console.warn('[Incident Injector] Could not create TrustedTypes policy, falling back:', e.message);
        }
        return null;
    })();
    ...
    // Attach button handlers + auto-refresh to the injected 500 page.
    // The page uses data-ii-action attributes instead of inline onclick= /
    // <script> so that sites with strict CSP (e.g. `script-src 'self'`) don't
    // block the button actions.
    function wireUp500Page() {
        try {
            document.querySelectorAll('[data-ii-action="reload"]').forEach((btn) => {
                btn.addEventListener('click', () => window.location.reload());
            });
            document.querySelectorAll('[data-ii-action="back"]').forEach((btn) => {
                btn.addEventListener('click', () => history.back());
            });
            setTimeout(() => {
                const refreshBtn = document.querySelector('[data-ii-action="reload"]');
                if (refreshBtn) {
            ...
            console.warn('[Incident Injector] Could not wire up 500 page actions:', e.message);
        }
    }
    ...
    function handleFormSubmission(event, clickedElement) {
        ...
        // Run scenario only if the Run Scenario trigger is enabled AND an
        // active scenario is selected. Previously this fired whenever an
        // active scenario existed, ignoring the toggle.
        if (runScenarioOnSubmit) {
            chrome.storage.sync.get(['active_scenario_id'], (result) => {
                ...
                if (hasActiveScenario) {
                    ...
                    console.log('[PagerDuty Simulator] Run Scenario enabled but no active scenario set');
                }
            });
        } else {
            console.log('[PagerDuty Simulator] Run Scenario trigger disabled, skipping');
        }
    ...
    function handleElementIncident(event, element) {
        ...
        if (runScenarioOnSubmit) {
            ...
            console.log('[PagerDuty Simulator] Run Scenario trigger disabled on click, skipping');
        }
        ...
        setTimeout(() => {
            ...
            if (redirectTo500) {
                ...
                try {
                        ...
                        <html lang="en">
                            ...
                            <body>
                                ...
                                <main class="main-content">
                                    ...
                                    <div class="actions">
                                        <button class="btn btn-primary" data-ii-action="reload">
                                            🔄 Try Again
                                        </button>
                                        <button class="btn btn-secondary" data-ii-action="back">
                                            ← Go Back
                                        </button>
                                    </div>
                                </main>
                    ...
                    document.body.innerHTML = toTrustedHTML(`
                        <div style="
                            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
                            background-color: #f5f7fa;
                            color: #2c3e50;
                        ...
                        ">
                            ...
                            <main style="max-width: 800px; margin: 80px auto; padding: 0 20px; text-align: center;">
                                ...
                                <div style="display: flex; gap: 16px; justify-content: center; margin-top: 40px;">
                                    <button data-ii-action="reload" style="padding: 12px 24px; border-radius: 8px; font-size: 16px; font-weight: 500; cursor: pointer; border: none; background: #3b82f6; color: white; transition: all 0.2s;">🔄 Try Again</button>
                                    <button data-ii-action="back" style="padding: 12px 24px; border-radius: 8px; font-size: 16px; font-weight: 500; cursor: pointer; background: #f3f4f6; color: #374151; border: 1px solid #d1d5db;">← Go Back</button>
                                </div>
                            </main>
